-- Job-agent tracker schema v2.
--
-- Changes from v1:
--   * companies extracted into its own table (FK from opportunities).
--   * opportunities columns realigned to OpportunityRecord
--     (source_url instead of url; posted_at; salary_* with REAL types;
--      raw_payload + search_context as JSON; date_discovered + last_seen_at).
--   * fit_score / sector / fit_reasoning moved out of opportunities into
--     a dedicated eval_decisions table (Phase 6).
--   * opportunities keeps notes/tags columns to preserve compatibility
--     with the update_opportunity_status(notes=...) public API and
--     legacy [size:X] tag handling in fit_scorer.
--
...
CREATE TABLE opportunities (
    ...
    -- TODO(phase-6-cleanup): notes/tags retained for v1 compatibility
    -- (fit_scorer writes [size:X] hint into notes; update_opportunity_status
    --  accepts notes=). When fit_scorer is replaced by OpportunityEvaluator,
    --  size lives at companies.size_bucket and notes/tags can be dropped.
    notes           TEXT,
    tags            TEXT
);
...
-- 4. Recruiters / agencies (verbatim from v1).
...
-- 5. Applications (resume_variant CHECK removed; profile-driven now).
...
-- 6. Status history (verbatim from v1).
...
-- 7. Communications (verbatim from v1).
...
-- 8. Events (added 'opportunity_seen_again' and 'company_upserted' types).
CREATE TABLE events (
    id              INTEGER PRIMARY KEY AUTOINCREMENT,
    occurred_at     TEXT NOT NULL,
