-- Job-agent tracker schema v2.13.
--
-- Changes from v2.12:
--   * Drop opportunities.notes (TEXT) — 0% populated in production
--     per Spec 6a TASK 6.0 audit (May 12, 2026); consumers
--     (generate_shortlist size-tag parsing, prompt_bundles markdown
--     "**Notes:**" block) removed in commits a247f69 / 3bb923b /
--     a803c5b.
--   * Drop opportunities.tags (TEXT) — 0% populated, zero consumers.
--   * Drop companies.size_label (TEXT) — 0% populated for business
--     logic; jobspy_client was the only writer (data discarded for
--     ~17K rows). Filed as architectural debt for future structured
--     size columns. companies.size_bucket is retained (different
--     column).
--
...
INSERT INTO schema_version (version) VALUES (213);
